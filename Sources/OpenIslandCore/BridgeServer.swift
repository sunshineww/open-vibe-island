...
public final class BridgeServer: @unchecked Sendable {
    ...
    private func handleClaudeHook(_ payload: ClaudeHookPayload, from clientID: UUID) {
        ...
        case .userPromptSubmit:
            ...
            // real-time `ClaudeTranscriptWatcher` already routed the
            // session to `.interrupted` via the transcript sentinel,
            // so we just need to clean up bookkeeping and progress
            // the new turn normally.
        ...
        case .postToolUseFailure:
            ...
            // is owned by `ClaudeTranscriptWatcher`, which tails the
            // JSONL transcript for the interrupt sentinel. This branch
            // only catches genuine tool-level failures that happen to
            // carry `is_interrupt: true`.
